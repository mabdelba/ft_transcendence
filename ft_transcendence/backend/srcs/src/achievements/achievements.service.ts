...
export class AchievementsService {
    ...
    async checkIfAchievements(userId: number, achievementId: number) {
        ...
        if (!checkIfAcquired.users[0])
        {
            ...
            return "You have acquired the achievement: " + (await getAchievementFromId(achievementId)).name;
        }
        else
            return "You already have the achievement: " + (await getAchievementFromId(achievementId)).name
    }
    async checkIfAchievementsAcquired(user: User) {
        if (user.numberOfGamesPlayed == 1)
            return await this.checkIfAchievements(user.id, 0);
        if ((user.level < 5)  && (user.level > 0))
                return await this.checkIfAchievements(user.id, 1);
        else if ((user.level < 10)  && (user.level >= 5))
                return await this.checkIfAchievements(user.id, 6);
        else if ((user.level < 15)  && (user.level >= 10))
                return await this.checkIfAchievements(user.id, 7);
        else if ((user.level < 22)  && (user.level >= 15))
                return await this.checkIfAchievements(user.id, 8);
        else if ((user.level < 30)  && (user.level >= 22))
                ...
                return await this.checkIfAchievements(user.id, 22);
    }
}
