  ...
  const handleSubmit = async (e: any) => {
    ...
    };
    {
      ...
      if (Lastname != Array[1]) {
        ...
        await axios
          .put(url, { lastname: Lastname }, config)
          .then(() => {
            const _user: User = user;
            _user.login = undefined;
            setUser(_user);
      ...
      {
        if (!avatarToUpload) setSelectFileError(false);
        else {
          ...
          <div className="w-full h-auto md:h-[42%]    mt-8 md:mt-0 px-4 md:px-8  xl:px-16 flex  flex-col md:flex-row justify-between  ">
            <div className=" w-[98%]  md:w-[49%] md:h-full h-auto  flex    flex-col ">
              <div className="w-full h-auto  md:h-1/2  flex flex-col justify-center">
                ...
                <span
                  className={`w-1/2 NeonShadow text-[5px] md:text-[12px]  flex font-light items-center ${
                    selectFileError ? '' : 'text-red-600 redShadow'
                  } `}
                >
                  ({filename})
                </span>
