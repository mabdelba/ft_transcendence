  ...
  return (
    <main className="w-screen h-screen flex  min-h-[600px] min-w-[280px] overflow-y-hidden">
      <div
        ...
        className={`h-full ${
          showSideBar ? '' : 'hidden'
        }  sm:block  w-20 xl:w-60  border-r-[3px] lineshad bg-opacity-10 bg-blue-500`}
      >
      ...
      <div className="w-full ">
        <div
          ...
          className="h-14 xl:h-16  border-b-[3px] lineshad bg-opacity-10 bg-blue-500 flex justify-between"
        >
          <BurgButton setFlag={setShowSideBar} val={showSideBar} />
          {!showSideBar && (
            <SearchBar
              setUsersResults={setUsersResults}
              setChannelsResults={setChannelsResults}
          ...
          <Link
            ...
            href={`/profil/${user.login}`}
            className="flex flex-row space-x-2 items-center min-w-[40px] w-auto justify-center  font-Orbitron font-light hover:text-lime-300 hover:font-extrabold text-xs md:text-base 2xl:text-lg pr-6"
          >
            ...
            {showPdp && (
              <Image
                ...
                className={`w-10 h-10 lineshad  ${
                  !hoverBool ? 'border-[2px]' : 'border-lime-300 border-[4px]'
                }  rounded-full`}
              />
            )}
          </Link>
        ...
        {((usersResults && usersResults.length > 0) ||
          (channelsResults &&
            ...
            !channelsResults.every((channel) => channel.isMember === true))) &&
          hideResults === false && (
            <ResultElements
              users={usersResults}
              channels={channelsResults}
              setChannels={setChannelsResults}
            />
          )}
