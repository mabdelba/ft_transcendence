...
function MyDropDown(props: {
  ...
  const setAsAdmin = () => {
    ...
    axios
      .post(apiUrl, { channelName: props.roomSelected, user: props.memberSelected }, config)
      .then(() => {
  ...
  const handleKick = () => {
    ...
    try {
      socket.emit('remove-user-from-channel', {
        ...
        myLogin: user.login,
        otherLogin: props.memberSelected,
      });
      const tempMembers = props.members;
      const index = tempMembers.findIndex((obj: any) => obj.login == props.memberSelected);
      if (index != -1) {
  ...
  const handleBan = () => {
    props.setOpenModal(false);
    socket.emit('ban-user-in-channel', {
      channelName: props.roomSelected,
      myLogin: user.login,
      otherLogin: props.memberSelected,
    });
  ...
  const handleMute = () => {
    props.setOpenModal(false);
    socket.emit('mute-user-in-channel', {
      channelName: props.roomSelected,
      myLogin: user.login,
      otherLogin: props.memberSelected,
    });
