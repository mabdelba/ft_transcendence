...
function MyMenu(props: {slected: number, setOpenMembers : any, setOpenSettings: any, setOpenInvite: any ,roomSelected : string, iAm?: string, channelType?: number, setShowArray : any, setRoomSelected: any, userId: number}) {
  ...
  const handleLeave = ()=> {
    if(socket){   
      socket.emit('remove-user-from-channel', {channelName: props.roomSelected, myLogin: user.login})
  ...
  const blockFriend = () => {
    ...
    if(index != -1){
      ...
      if(_user.blockedList != undefined)
      _user.blockedList.push(newBlocked)
  ...
  const friendMenu = [
    ...
    { handleClick: blockFriend, label: 'Block' ,render: () => {return(<ImBlocked size="20"/>)}},
    { handleClick: handleInvite, label: 'Invite for a match' ,render: () => {return(<GiGamepad size="20"/>)} },
  ]
  
  const GroupMenu = [
    { handleClick: ()=>{ props.setOpenSettings(true) }, label: 'Group settings' ,render: () => {return(<FiSettings size="20"/>)}},
    { handleClick: handleLeave, label: 'Leave' ,render: () => {return(<TbLogout2 size="20"/>)}},
