...
function SearchBar(props: ButtonProps) {
  ...
  return (
	<Link 
		...
		href={props.path} className={` flex flex-col   w-full h-full transition duration-300 ${props.drp ? 'bg-black lineshad bg-opacity-25' : ''} font-Orbitron min-h-[74px] justify-center hover:text-lime-300 hover:font-extrabold`}>
		<div className='flex flex-col justify-end parent'>
			<div className=' flex'>
				<div className='min-w-[76px] min-h-[74px] flex items-center'>
					{props.icon && props.icon2 && <Image className='m-auto' src={!hoverBool ? props.icon : props.icon2} alt='icon' />}
				</div>
