...
const gamePage = () => {
  ...
  useEffect(() => {
    ...
    user.socket?.on(
      'GameState',
      (data: { player1: Matter.Vector; player2: Matter.Vector; ball: Matter.Vector }) => {
  ...
  useEffect(() => {
    if (gameDiv.current) {
      ...
      if (gameSocket) game.setSocket(gameSocket);
      game.start();
    }
    return () => {
      game?.destroy();
      ...
      <div className="font-Orbitron w-screen h-screen flex flex-col justify-center items-center">
        ...
        {startState && (
          <button
            ...
            className="z-10 mb-2 w-fit NeonShadowBord flex flex-row items-center justify-center h-fit px-4 py-3 hover:bg-white hover:text-[black] transition-[300]"
          >
            Start Game
          </button>
        )}
