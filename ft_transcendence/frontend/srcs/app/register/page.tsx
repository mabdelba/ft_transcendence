...
function Register() {
  ...
  const handleSubmit = (event: any) => {
    ...
    if (Data.firstname == '') return setFerror(false);
    if (Data.lastname == '') return setLerror(false);
    if (Data.username == '') return setUerror(false);
    if (Data.email == '') return setEerror(false);
    if (Data.password == '') return setPerror(false);
    if (!ferror || !lerror || !uerror || !eerror || !perror || !verror) return;
    console.log(Data);
  };

  const handleBlur = () => {
    const regFirstName = Regex.test(Data.firstname);
    if (!regFirstName && Data.firstname != '') setFerror(false);
    else setFerror(true);
    if (Data.username != '') setUerror(true);
    const regLastName = Regex.test(Data.lastname);
    if (!regLastName && Data.lastname != '') setLerror(false);
    else setLerror(true);
    const regEmail = EmRegex.test(Data.email);
    if (!regEmail && Data.email != '') setEerror(false);
    else setEerror(true);
    if (!PassRegex.test(Data.password) && Data.password != '') setPerror(false);
    else setPerror(true);
    if (vpassword !== password) setVerror(false);
    else setVerror(true);
  };
